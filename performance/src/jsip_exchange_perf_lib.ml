...
let assoc_tests ~name ~create ~set ~get ~key_of_index =
  ...
  List.concat_map sizes ~f:(fun n ->
    let prebuilt = build n in
    (* Keys are prebuilt so [get_*] times only the lookup, no [key_of_index]
       (string formatting / record construction). *)
...
let bench_allocation =
  List.concat_map sizes ~f:(fun n ->
    ...
    let target = present_key n in
    let is_target x = Int.equal x target in
    [ Bench.Test.create
    ...
    ; Bench.Test.create
        ...
        (fun () ->
           ignore
             (Allocations.First_match.non_silly input ~f:is_target
              : int option))
    ])
;;
