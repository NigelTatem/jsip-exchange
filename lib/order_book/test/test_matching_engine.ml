...
let%expect_test "exact cross at same price" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x100 aggressor=2(Alice) BUY 1 resting=Bob(0) 0
    |}]
;;
...
let%expect_test "partial fill: buy is larger than resting sell" =
  ...
  [%expect
    ...
    ACCEPTED id=2 AAPL BUY 100@$150.00 DAY
    FILL fill_id=1 AAPL $150.00 x60 aggressor=2(Alice) BUY 1 resting=Bob(0) 0
    |}];
...
let%expect_test "aggressor sweeps multiple resting orders" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x50 aggressor=3(Alice) BUY 1 resting=Bob(0) 0
    FILL fill_id=2 AAPL $150.00 x50 aggressor=3(Alice) BUY 2 resting=Charlie(0) 0
    |}]
;;
...
let%expect_test "IOC: partial fill then cancel remainder" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x40 aggressor=2(Alice) BUY 1 resting=Bob(0) 0
...
let%expect_test "price priority: naive impl matches first-found, not best" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $10.00 x100 aggressor=3(Alice) BUY 1 resting=Charlie(0) 0
    |}]
;;
...
let%expect_test "scenario: two participants trade, book reflects state" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.10 x50 aggressor=5(Charlie) BUY 3 resting=Bob(0) 1
    === AAPL ===
      BIDS:
        $149.90 x100
        $149.80 x200
...
let%expect_test "scenario: aggressive IOC sweeps entire book" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x50 aggressor=4(Alice) BUY 1 resting=Bob(0) 0
    FILL fill_id=2 AAPL $150.10 x50 aggressor=4(Alice) BUY 2 resting=Charlie(0) 0
    FILL fill_id=3 AAPL $150.20 x50 aggressor=4(Alice) BUY 3 resting=Bob(0) 1
...
let%expect_test "scenario: fill IDs are globally sequential" =
  ...
  [%expect
    ...
    ACCEPTED id=2 TSLA SELL 100@$200.00 DAY
    ACCEPTED id=3 AAPL BUY 100@$150.00 DAY
    FILL fill_id=1 AAPL $150.00 x100 aggressor=3(Alice) BUY 1 resting=Bob(0) 0
...
let%expect_test "Scenario: Submit with client order ID then cancel with ID." =
  ...
  [%expect
    {|
    CANCELLED id=1 AAPL remaining=100 reason=PARTICIPANT_REQUESTED
    BBO AAPL bid=- ask=-
    |}]
;;
...
let%expect_test "Scenario: Submit with a duplicate client order ID." =
  ...
  Harness.submit_ t (Harness.buy ~price_cents:15000 ~client_order_id:1 ());
  [%expect
    {| REJECTED AAPL BUY 100@$150.00 reason=duplicate client order id |}]
;;
...
let%expect_test "Scenario: Cancel an already-filled order." =
  ...
  [%expect
    {|
    ACCEPTED id=2 AAPL SELL 100@$150.00 DAY
    FILL fill_id=1 AAPL $150.00 x100 aggressor=2(Bob) SELL 1 resting=Alice(2) 1
