...
let%expect_test "single buy order, nothing to match" =
  ...
  [%expect {| ACCEPTED id=1 0 BUY 100@$150.00 DAY |}]
;;
...
let%expect_test "two orders that don't cross" =
  ...
  [%expect
    ...
    ACCEPTED id=2 0 SELL 100@$151.00 DAY
    |}]
;;
...
let%expect_test "exact cross at same price" =
  ...
  [%expect
    ...
    FILL fill_id=1 0 $150.00 x100 aggressor=2(Alice) BUY 1 resting=Bob(0) 0
    |}]
;;
...
let%expect_test "buy crosses at resting price, not aggressor price" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 SELL 100@$150.00 DAY
    ACCEPTED id=2 0 BUY 100@$151.00 DAY
    FILL fill_id=1 0 $150.00 x100 aggressor=2(Alice) BUY 1 resting=Bob(0) 0
    |}]
;;
...
let%expect_test "partial fill: buy is larger than resting sell" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 SELL 60@$150.00 DAY
    ACCEPTED id=2 0 BUY 100@$150.00 DAY
    FILL fill_id=1 0 $150.00 x60 aggressor=2(Alice) BUY 1 resting=Bob(0) 0
    |}];
...
let%expect_test "aggressor sweeps multiple resting orders" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 SELL 50@$150.00 DAY
    ACCEPTED id=2 0 SELL 80@$150.00 DAY
    ACCEPTED id=3 0 BUY 100@$150.00 DAY
    FILL fill_id=1 0 $150.00 x50 aggressor=3(Alice) BUY 1 resting=Bob(0) 0
    FILL fill_id=2 0 $150.00 x50 aggressor=3(Alice) BUY 2 resting=Charlie(0) 0
    |}]
;;
...
let%expect_test "IOC: no match means immediate cancel" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 BUY 100@$150.00 IOC
    CANCELLED id=1 0 remaining=100 reason=IOC_REMAINDER
    |}]
;;
...
let%expect_test "IOC: partial fill then cancel remainder" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 SELL 40@$150.00 DAY
    ACCEPTED id=2 0 BUY 100@$150.00 IOC
    FILL fill_id=1 0 $150.00 x40 aggressor=2(Alice) BUY 1 resting=Bob(0) 0
    CANCELLED id=2 0 remaining=60 reason=IOC_REMAINDER
    |}]
;;
...
let%expect_test "orders for different symbols don't cross" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 SELL 100@$150.00 DAY
    ACCEPTED id=2 1 BUY 100@$150.00 DAY
    === 0 ===
      BIDS: (empty)
      ASKS:
        $150.00 x100
...
let%expect_test "price priority: naive impl matches first-found, not best" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 SELL 100@$10.00 DAY
    ACCEPTED id=2 0 SELL 100@$10.05 DAY
    ACCEPTED id=3 0 BUY 100@$10.05 DAY
    FILL fill_id=1 0 $10.00 x100 aggressor=3(Alice) BUY 1 resting=Charlie(0) 0
    |}]
;;
...
let%expect_test "BBO update emitted when order rests on book" =
  ...
  let events =
    Harness.submit_quiet
      t
      (Harness.sell ~price_cents:15100 ~client_order_id:1 ())
  in
  Harness.print_events ~show:show_bbo events;
...
let%expect_test "scenario: two participants trade, book reflects state" =
  ...
  submit_ t (Harness.buy ~price_cents:14990 ~size:100 ~client_order_id:1 ());
  submit_ t (Harness.buy ~price_cents:14980 ~size:200 ~client_order_id:2 ());
  submit_
    t
    (Harness.sell
       ~price_cents:15010
  ...
  [%expect
    {|
    ACCEPTED id=1 0 BUY 100@$149.90 DAY
    ACCEPTED id=2 0 BUY 200@$149.80 DAY
    ACCEPTED id=3 0 SELL 100@$150.10 DAY
    ACCEPTED id=4 0 SELL 150@$150.20 DAY
    ACCEPTED id=5 0 BUY 50@$150.10 DAY
    FILL fill_id=1 0 $150.10 x50 aggressor=5(Charlie) BUY 3 resting=Bob(0) 1
    === 0 ===
      BIDS:
        $149.90 x100
        $149.80 x200
...
let%expect_test "scenario: aggressive IOC sweeps entire book" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 SELL 50@$150.00 DAY
    ACCEPTED id=2 0 SELL 50@$150.10 DAY
    ACCEPTED id=3 0 SELL 50@$150.20 DAY
    ACCEPTED id=4 0 BUY 200@$150.20 IOC
    FILL fill_id=1 0 $150.00 x50 aggressor=4(Alice) BUY 1 resting=Bob(0) 0
    FILL fill_id=2 0 $150.10 x50 aggressor=4(Alice) BUY 2 resting=Charlie(0) 0
    FILL fill_id=3 0 $150.20 x50 aggressor=4(Alice) BUY 3 resting=Bob(0) 1
    CANCELLED id=4 0 remaining=50 reason=IOC_REMAINDER
    === 0 ===
      BIDS: (empty)
      ASKS: (empty)
      BBO: - / -
...
let%expect_test "scenario: order IDs are globally sequential" =
  ...
  [%expect
    {|
    ACCEPTED id=1 0 BUY 100@$150.00 DAY
    ACCEPTED id=2 1 SELL 100@$200.00 DAY
    ACCEPTED id=3 2 BUY 100@$280.00 DAY
    |}]
;;
...
let%expect_test "scenario: fill IDs are globally sequential" =
  ...
  [%expect
    ...
    ACCEPTED id=2 1 SELL 100@$200.00 DAY
    ACCEPTED id=3 0 BUY 100@$150.00 DAY
    FILL fill_id=1 0 $150.00 x100 aggressor=3(Alice) BUY 1 resting=Bob(0) 0
    ACCEPTED id=4 1 BUY 100@$200.00 DAY
    FILL fill_id=2 1 $200.00 x100 aggressor=4(Alice) BUY 2 resting=Charlie(0) 0
    |}]
;;
...
let%expect_test "Scenario: Submit with client order ID then cancel with ID." =
  ...
  [%expect
    {|
    CANCELLED id=1 0 remaining=100 reason=PARTICIPANT_REQUESTED
    BBO 0 bid=- ask=-
    |}]
;;
...
let%expect_test "Scenario: Submit with a duplicate client order ID." =
  ...
  [%expect
    ...
    |}];
  Harness.submit_ t (Harness.buy ~price_cents:15000 ~client_order_id:1 ());
  [%expect {| REJECTED 0 BUY 100@$150.00 reason=duplicate client order id |}]
;;
...
let%expect_test "Scenario: Cancel an already-filled order." =
  ...
  [%expect
    {|
    ACCEPTED id=2 0 SELL 100@$150.00 DAY
    FILL fill_id=1 0 $150.00 x100 aggressor=2(Bob) SELL 1 resting=Alice(2) 1
    TRADE 0 $150.00 x100
    BBO 0 bid=- ask=-
    |}];
  let events =
  ...
  Harness.print_events events;
  [%expect
    {| CANCEL_REJECTED participant=Alice client_id=1 reason=unknown order |}]
;;
